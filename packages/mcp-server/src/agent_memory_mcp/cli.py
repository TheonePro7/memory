...
def cmd_recall() -> None:
    try:
        ...
        # 写一个最小 context 文件避免下游出错
        pid = os.getpid()
        tmp = Path.home() / ".agent-memory" / f"context.{pid}.json"
        tmp.parent.mkdir(parents=True, exist_ok=True)
        tmp.write_text(json.dumps({"mem0": [], "recent_sessions": [], "active_tasks": []}), encoding="utf-8")
...
def cmd_summarize() -> None:
    try:
        ...
        print(f"summarize 失败: {e}", file=sys.stderr)
...
def cmd_remember() -> None:
    ...
    try:
        ...
        print(f"remember 失败: {e}", file=sys.stderr)
        sys.exit(1)
