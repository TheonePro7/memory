...
def scan_agents(refresh: bool = Query(False, description="强制重新扫描，不使用缓存")):
    ...
    try:
        ...
        logger.exception("Agent 扫描失败")
        return JSONResponse(status_code=500, content={"error": f"Agent 扫描失败: {str(e)}"})
...
def list_agents():
    ...
    try:
        ...
        return JSONResponse(status_code=500, content={"error": f"Agent 列表加载失败: {str(e)}"})
...
def update_custom_agent(agent_id: str, update: CustomAgentUpdate):
    ...
    try:
        ...
        if not result:
            return JSONResponse(status_code=404, content={"error": "自定义 Agent 不存在"})
        return result
    except Exception as e:
