...
仅规则驱动，不做 LLM 分类。规则不命中时默认走 mem0 语义搜索。

#### → mem0（语义/事实/偏好）

**显式记住指令**
```
记住 | 请注意 | 以后要知道 | 牢记 | 永远记住
保存 | 存一下 | 记下来 | 别忘了 | 记录 | 保留 | 标记
```

**偏好/规则/约定**
```
用 | 要用 | 不要用 | 首选用 | 请用 | 建议用 | 倾向于
偏好 | 喜欢 | 习惯 | 风格 | 命名 | 格式
规范 | 规则 | 约定 | 标准 | 推荐
```
> 例: "用 2 空格缩进"、"不要用 any 类型"、"命名规范是 camelCase"
...
依赖 | 版本 | 地址 | 端口
```
> 例: "这个项目用 PostgreSQL"、"依赖 Redis 7.2"

**解法/知识**
```
怎么解决 | 怎么修 | 如何修复 | 如何解决 | 怎么处理
解法 | 解决方案 | 解决办法 | 修复方案 | 最佳实践
之前怎么处理 | 当时怎么搞 | 有没有遇到过 | 遇到过吗
```
...
**决策回溯**
```
怎么决定 | 为什么选 | 为什么决定 | 决策原因
试过 | 尝试过 | 踩过坑
回顾 | 复盘 | 回看 | 查看历史
```
> 例: "上次为什么选 Apollo Server"、"当时踩过什么坑"
