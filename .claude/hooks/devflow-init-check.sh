...
if [ "$bd_ok" -ne 1 ] || [ "$gitnexus_ok" -ne 1 ]; then
  printf '{"systemMessage":"devflow: Phase 3 auto-setup in progress...","hookSpecificOutput":{"hookEventName":"SessionStart","additionalContext":"devflow Phase 3 pending — auto-installing tools and initializing project."}}'
else
  printf '{"hookSpecificOutput":{"hookEventName":"SessionStart","additionalContext":"devflow Phase 3 ready — beads + gitnexus + autoresearch initialized. devflow 5-phase orchestrator available."}}'
fi
